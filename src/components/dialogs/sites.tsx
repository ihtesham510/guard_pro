...
import { type SiteInsertSchemaWithAddress, siteInsertSchemaWithAddress } from '@/services/site.schema'
...
import { addSite, uploadSitePictures, addSitePictures } from '@/services/site.api'
...
export function SiteDialog() {
	...
	}, [dialogs.state['add-site']])

	return (
		<ResponsiveDialog open={dialogs.state['add-site']} onOpenChange={e => dialogs.setState('add-site', e)}>
			<ResponsiveDialogContent>
				<ResponsiveDialogForm form={form} onSubmit={form.handleSubmit(handleSubmit)}>
					...
					{!companies.isLoading && (!companies.data || companies.data?.length === 0) && (
						<div className='flex flex-col justify-center gap-2 mt-5'>
							<Alert variant='destructive'>
								<CircleX />
								<AlertTitle>No Companies Present</AlertTitle>
								<AlertDescription>
									A company is required before adding a site, please add a company before adding sites for that company.
								</AlertDescription>
							</Alert>
							<Button
								onClick={() => {
						...
						<Stepper hideSteps={isMobile}>
							...
							<Step>
								<FieldSet>
									...
									<FieldDescription>Upload pictures of the site (optional).</FieldDescription>
									<FormItem>
